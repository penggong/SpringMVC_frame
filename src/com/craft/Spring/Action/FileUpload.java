...
import com.craft.Spring.util.FileHelpUtil;
...
public class FileUpload {
     ...
     */
    @RequestMapping(value = "fileUpload_multipartFile.action")
    @ResponseBody
    public String fileUpload_multipartFile(HttpServletRequest request, @RequestParam("file_upload") MultipartFile multipartFile) {
        ...
        String filePath = FileHelpUtil.FilesUpload_transferTo_spring(request, multipartFile, "Resource\\fileUpload");
     ...
     */
    @RequestMapping(value = "fileUpload_multipartRequest.action")
    @ResponseBody
     ...
     * 自动扫描全部的input表单
     */
    @RequestMapping(value = "fileUpload_CommonsMultipartResolver.action")
    @ResponseBody
    public String fileUpload_CommonsMultipartResolver(HttpServletRequest request) {
        ...
        if (multipartResolver.isMultipart(request)) {
            ...
            while (iter.hasNext()) {
                ...
                //调用保存文件的帮助类进行保存文件，并返回文件的相对路径
                String fileName = FileHelpUtil.FilesUpload_transferTo_spring(request, multipartFile, "Resource\\fileUpload");
     ...
     * 通过流的方式上传文件
     */
    @RequestMapping("fileUpload_stream.action")
    @ResponseBody
    public String upFile(HttpServletRequest request, @RequestParam("file_upload") MultipartFile multipartFile){
        String filePath= FileHelpUtil.FilesUpload_stream(request,multipartFile,"Resource\\fileUpload");
        return "{'TFMark':'true','Msg':'upload success !','filePath':'" + filePath + "'}";
    }
        ...
        if (multipartFile != null && multipartFile.length > 0) {
            ...
            try {
                for (int i = 0; i < multipartFile.length; i++) {
                    ...
                    //保存文件
                    String fileName = FileHelpUtil.FilesUpload_transferTo_spring(request, file, "F:\\opt");
                    System.out.println(fileName);
                }
    ...
    public void fileDownload_servlet(HttpServletRequest request, HttpServletResponse response) {
        FileHelpUtil.FilesDownload_stream(request,response,"/files/download/mst.txt");
    }
}
